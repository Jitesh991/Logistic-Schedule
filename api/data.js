// api/data.js — Protected CRUD for schedules, trucks, customers, drivers
// All requests require a valid Bearer token from /api/auth
// HR Viewers (role: 'viewer') get read-only access
...
// ── Token verify (mirror of auth.js) ────────────────
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    if (caller.role === 'viewer') {
      return res.status(403).json({ error: 'Read-only access — contact your admin to make changes' });
    }
